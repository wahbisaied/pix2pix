...
print("=== Model 200 Evaluation Pipeline (Simple) ===")

# Step 1: Convert reference images
...
# Step 2: Setup model using CycleGAN framework
print("Step 2: Setting up model...")
import sys
original_argv = sys.argv
sys.argv = [
    'evaluate_model_200_simple.py', 
...
# Step 3: Create dataset and generate fake images
...
# Copy some trainA files to test directory
train_a_dir = os.path.join(DATASET_DIR, "trainA")
if os.path.exists(train_a_dir):
    import shutil
    train_files = [f for f in os.listdir(train_a_dir) if f.endswith('.nii.gz')][:50]
